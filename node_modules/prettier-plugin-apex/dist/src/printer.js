...
function handleIfElseBlock(path, print) {
    ...
    const ifBlockContainsPrettierIgnore = node.ifBlocks.map((ifBlock) => ifBlock.comments?.some((comment) => comment.leading && (0, comments_1.isPrettierIgnore)(comment)));
    let lastIfBlockHardLineInserted = false;
    ifBlockDocs.forEach((ifBlockDoc, index) => {
        if (index > 0) {
            parts.push(ifBlockContainsPrettierIgnore[index]
                ? hardline
                : concat([
    ...
    if (elseBlockDoc) {
        ...
        const elseBlockContainsPrettierIgnore = node.elseBlock?.value?.comments?.some((comment) => comment.leading && (0, comments_1.isPrettierIgnore)(comment));
